...
export const SkillSelector = ({ 
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === ',') {
      ...
      if (trimmed && skills.length < 5 && !skills.some(s => s.name.toLowerCase() === trimmed.toLowerCase())) {
        onAdd(trimmed);
        setInput('');
      }
    }
        ...
        {skills.length < 5 && (
          <div className={cn(
            ...
            <Field
              ...
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={handleKeyDown}
              placeholder={placeholder || "Add a skill (e.g. React, Python)..."}
              className="bg-transparent border-none focus:ring-0 h-14 pt-0 pb-0 shadow-none text-xl"
            />
            <div className="pr-6 hidden md:block">
              <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white/10 group-focus-within/add:text-primary/40 transition-colors">Press Enter to Add</span>
            </div>
          </div>
        )}
      </div>
