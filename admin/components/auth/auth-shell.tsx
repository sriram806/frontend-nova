...
export function AuthShell({
  ...
  subtitle,
  children,
  preTitle = "Administrative Access",
  leftPanelTitle = "Nova Intelligence",
  leftPanelSubtitle = "Next-Gen Platform Management",
  leftPanelFooter = "Control everything from a single, high-performance interface.",
  backgroundImage = "/auth-bg.png",
  topCta
}: AuthShellProps) {
  return (
    <main className="flex min-h-screen w-full bg-[#030712] text-white overflow-hidden relative">
      {/* Background Ambient Glows */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-primary/20 blur-[120px] rounded-full animate-glow" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-600/10 blur-[120px] rounded-full animate-glow" style={{ animationDelay: '2s' }} />
      ...
      <section className="hidden w-1/2 p-4 lg:block relative z-10">
        <div className="relative h-full w-full overflow-hidden rounded-[2.5rem] bg-[#0A1024] shadow-2xl border border-white/5">
          <Image
            ...
            alt="Auth background"
            fill
            className="object-cover object-center transition-transform duration-1000 hover:scale-110 opacity-60"
            priority
          />
          ...
          <div className="absolute bottom-16 left-0 right-0 px-12 z-10">
            <motion.div 
              ...
              className="max-w-md mx-auto space-y-6 backdrop-blur-md bg-white/5 p-10 rounded-[2.5rem] border border-white/10 shadow-2xl relative overflow-hidden group"
            >
              ...
              <div className="flex items-center gap-4 pt-4">
                 <div className="flex -space-x-3">
                    {[1,2,3].map(i => (
                      <div key={i} className="h-8 w-8 rounded-full border-2 border-[#0A1024] bg-white/10" />
                    ))}
                 </div>
                 <span className="text-xs text-white/40 font-bold tracking-wider uppercase">Trusted by Global Teams</span>
              </div>
            </motion.div>
          </div>
      ...
      <section className="flex w-full flex-col items-center justify-center p-6 lg:w-1/2 lg:p-12 relative z-10">
        ...
        <div className="w-full max-w-[440px]">
          ...
          >
            <span className="inline-block px-4 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-[0.2em] mb-6 border border-primary/20">
              {preTitle}
            </span>
        ...
        {/* Footer info */}
        <div className="mt-20 text-center opacity-30 text-[11px] font-bold uppercase tracking-[0.3em]">
           &copy; 2026 Nova Platforms &bull; Security Grade A+
        </div>
      </section>
    </main>
  );
